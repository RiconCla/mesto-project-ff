...
function handlerEditProfile(evt){
  ...
  .finally(() => {
    renderLoading({buttonElement:formProfileSubmitButton,isLoading:false});
  })
};
...
Promise.all([getUserInfo(), getInitialCards()])
  .then(([{ name, about, avatar, ['_id']:myUserId }, cardsData]) => {
    ...
    cardsData.forEach((card) => {
      placesList.append(addCard(card, setDeleteConfirmation, handlercardLike, openImageModal, myUserId, modalDeleteCard))
    });
    formNewCard.addEventListener('submit', (evt) => handlerAddNewCard(evt, myUserId));
  })
  .catch((err) => {
...
function handlerAddNewCard(evt, myUserId) {
  ...
  .finally(() => {
    renderLoading({buttonElement:formNewCardSubmitButton,isLoading:false});
  })
...
function renderNewCard(card, myUserId) {
  placesList.prepend(addCard(card, setDeleteConfirmation, handlercardLike, openImageModal, myUserId, modalDeleteCard));
}
...
function handlercardLike (likeButton, cardId, countLike, myUserId) {
...
function handlerUpdateAvatar (evt){
  ...
  .finally(() => {
    renderLoading({buttonElement:profileImageFormSubmitButton,isLoading:false});
  })
};
